...
@section('content')
                    ...
                    <div class="w-100">
                        <div class="row">
                            <div class="col">
                                ...
                                <h2 class="mb-0">{{ $empleado->nombres }}</h2>
                            </div>
                            <div class="col">
                                <h5 class="text-secondary mb-0">Puesto</h5>
                                <h2 class="mb-0">{{ $empleado->puesto }}</h2>
                            </div>
                            <div class="col">
                                ...
                                <h2 class="mb-0">{{ $empleado->fecha_contratacion }}</h2>
                            </div>
                            <div class="col">
                                <h5 class="text-secondary mb-0">
                                ...
                                <h2 class="mb-0">{{ $empleado->telefono }}</h2>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal fade" id="editModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        ...
        <div class="modal-dialog">
            <form action="{{ route('empleados.update', $empleado) }}" method="POST" class="modal-content">
                @csrf
                @method('PUT')
                ...
                <div class="modal-body bg-body-secondary">
                    ...
                    <label for="puesto" class="text-secondary fs-5 ms-1">Puesto</label>
                    <input type="text" name="puesto" class="form-control form-control-lg mb-2"
                        value="{{ $empleado->puesto }}" required>
                    <label for="nombres" class="text-secondary fs-5 ms-1">Fecha de Contratación</label>
                    <input type="date" name="fecha_contratacion" class="form-control form-control-lg mb-2"
                        value="{{ $empleado->fecha_contratacion }}" required>
                    ...
                    <input type="tel" name="telefono" class="form-control form-control-lg"
                        value="{{ $empleado->telefono }}" minlength="10" maxlength="10" pattern="^[0-9]*$" required>
                </div>
    ...
    <div class="modal fade" id="deleteModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
        aria-labelledby="editModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form action="{{ route('empleados.destroy', $empleado) }}" method="POST" class="modal-content">
                @csrf
                @method('DELETE')
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <h5 class="fw-normal">¿Seguro que deseas eliminar al empleado <b>{{ $empleado->nombres }}</b>?</h5>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary me-auto" data-bs-dismiss="modal">Cerrar</button>
